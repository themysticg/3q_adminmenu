...
RegisterNetEvent("Admin:Bennys", function(Result)
    ...
    TriggerServerEvent('3q_admin/server/open-bennys', Result['player'])
end)

RegisterNetEvent("Admin:Kill", function(Result)
   
    TriggerServerEvent('3q_admin/server/kill', Result['player'])
end)

RegisterNetEvent("Admin:Set:Environment", function(Result)
   
    TriggerServerEvent('3q_admin/server/set-environment', Result['weather'], Result['hour'], Result['minute'])
end)

RegisterNetEvent("Admin:Delete:Area", function(Result)
   
    TriggerServerEvent('3q_admin/server/delete-area', Result['type'], Result['radius'])
end)

RegisterNetEvent("Admin:Infinite:Ammo", function(Result)
   
    TriggerServerEvent('3q_admin/server/toggle-infinite-ammo', Result['player'])
end)

RegisterNetEvent("Admin:Infinite:Stamina", function(Result)
   
    TriggerServerEvent('3q_admin/server/toggle-infinite-stamina', Result['player'])
end)

RegisterNetEvent("Admin:Cloak", function(Result)
   
    TriggerServerEvent('3q_admin/server/toggle-cloak', Result['player'])
end)

RegisterNetEvent("Admin:Godmode", function(Result)
   
    TriggerServerEvent('3q_admin/server/toggle-godmode', Result['player'])
end)

RegisterNetEvent('Admin:Toggle:Noclip', function(Result)
   
    TriggerEvent('3q_admin/client/force-close')
    SendNUIMessage({
...
RegisterNetEvent('Admin:Teleport', function(Result)
    ...
    TriggerEvent('3q_admin/client/force-close')
    TriggerServerEvent('3q_admin/server/teleport-player', Result['player'], Result['type'])
end)

RegisterNetEvent("Admin:Chat:Say", function(Result)
   
    TriggerServerEvent('3q_admin/server/chat-say', Result['message'])
end)

RegisterNetEvent('Admin:Open:Clothing', function(Result)
   
    TriggerEvent('3q_admin/client/force-close')
    TriggerServerEvent('3q_admin/server/open-clothing', Result['player'])
end)

RegisterNetEvent('Admin:Revive:Radius', function(Result)
   
    TriggerServerEvent('3q_admin/server/revive-in-distance', Result['radius'])
end)

RegisterNetEvent('Admin:Revive', function(Result)
   
    TriggerServerEvent('3q_admin/server/revive-target', Result['player'])
end)

RegisterNetEvent('Admin:Remove:Stress', function(Result)
   
    TriggerServerEvent('3q_admin/server/remove-stress', Result['player'])
end)
...
RegisterNetEvent('Admin:Change:Model', function(Result)
    ...
    if Result['model'] ~= '' then
        ...
        if IsModelValid(Model) then
            TriggerServerEvent('3q_admin/server/set-model', Result['player'], Model)
        end
    end
end)
...
RegisterNetEvent('Admin:Reset:Model', function(Result)
    ...
    TriggerServerEvent('3q_admin/server/reset-skin', Result['player'])
end)

RegisterNetEvent('Admin:Armor', function(Result)
   
    TriggerServerEvent('3q_admin/server/set-armor', Result['player'])
end)

RegisterNetEvent('Admin:Food:Drink', function(Result)
   
    TriggerServerEvent('3q_admin/server/set-food-drink', Result['player'])
end)

RegisterNetEvent('Admin:Request:Gang', function(Result)
   
    TriggerServerEvent('3q_admin/server/request-gang', Result['player'], Result['gang'] ~= '' and Result['gang'] or 'none')
end)

RegisterNetEvent('Admin:Request:Job', function(Result)
   
    TriggerServerEvent('3q_admin/server/request-job', Result['player'], Result['job'] ~= '' and Result['job'] or 'unemployed')
end)

RegisterNetEvent("Admin:Drunk", function(Result)
   
    TriggerServerEvent('3q_admin/server/drunk', Result['player'])
end)

RegisterNetEvent("Admin:Animal:Attack", function(Result)
   
    TriggerServerEvent('3q_admin/server/animal-attack', Result['player'])
end)

RegisterNetEvent('Admin:Set:Fire', function(Result)
   
    TriggerServerEvent('3q_admin/server/set-fire', Result['player'])
end)

RegisterNetEvent('Admin:Fling:Player', function(Result)
   
    TriggerServerEvent('3q_admin/server/fling-player', Result['player'])
end)

RegisterNetEvent("Admin:Freeze:Player", function(Result)
   
    TriggerServerEvent('3q_admin/server/freeze-player', Result['player'])
end)

RegisterNetEvent('Admin:SetMoney', function(Result)
   
    TriggerServerEvent('3q_admin/server/set-money', Result['player'], Result['moneytype'], Result['amount'])
end)

RegisterNetEvent('Admin:GiveMoney', function(Result)
   

    TriggerServerEvent('3q_admin/server/give-money', Result['player'], Result['moneytype'], Result['amount'])
end)

RegisterNetEvent('Admin:GiveItem', function(Result)
   

    TriggerServerEvent('3q_admin/server/give-item', Result['player'], Result['item'], Result['amount'])
end)

RegisterNetEvent('Admin:Ban', function(Result)
   
    TriggerServerEvent('3q_admin/server/ban-player', Result['player'], Result['expire'], Result['reason'])
end)

RegisterNetEvent('Admin:Unban', function(Result)
    
    TriggerServerEvent("3q_admin/server/unban-player", Result['player'])
end)

RegisterNetEvent('Admin:Kick', function(Result)
    
    TriggerServerEvent('3q_admin/server/kick-player', Result['player'], Result['reason'])
end)

RegisterNetEvent('Admin:Kick:All', function(Result)
   
   TriggerServerEvent('3q_admin/server/kick-all-players', Result['reason'])
end)
...
RegisterNetEvent("Admin:Fart:Player", function(Result)
    ...
    TriggerServerEvent('3q_admin/server/play-sound', Result['player'], Result['fart'])
end)

RegisterNetEvent('Admin:Toggle:PlayerBlips', function()
   
    BlipsEnabled = not BlipsEnabled
    TriggerServerEvent('3q_admin/server/toggle-blips')
    SendNUIMessage({
        Action = "SetItemEnabled",
        Name = 'playerblips',
        State = BlipsEnabled
...
RegisterNetEvent('Admin:Toggle:Spectate', function(Result)
    ...
    if not SpectateEnabled then
        TriggerServerEvent('3q_admin/server/start-spectate', Result['player'])
    else
        ToggleSpectate(StoredTargetPed)
        TriggerServerEvent('3q_admin/server/stop-spectate')
    end
end)
...
RegisterNetEvent("3q_admin/client/delete-area", function(Type, Radius)
    Type = Type:lower()
...
RegisterNetEvent("3q_admin/client/freeze-player", function(Bool)
    FreezeEntityPosition(GetPlayerPed(PlayerId()), Bool)
end)

RegisterNetEvent("3q_admin/client/toggle-infinite-ammo", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent("3q_admin/client/toggle-infinite-stamina", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent("3q_admin/client/toggle-cloak", function(Bool)
    if Bool then
...
RegisterNetEvent("3q_admin/client/toggle-godmode", function(Bool)
    while Bool do
        Wait(1)
...
RegisterNetEvent('3q_admin/client/teleport-player', function(Coords)
    local Entity = PlayerPedId()    
    SetPedCoordsKeepVehicle(Entity, Coords.x, Coords.y, Coords.z)
end)

RegisterNetEvent('3q_admin/client/set-model', function(Model)
    QBCore.Functions.LoadModel(Model)
    SetPlayerModel(PlayerId(), Model)
    SetPedComponentVariation(PlayerPedId(), 0, 0, 0, 0)
end)

RegisterNetEvent('3q_admin/client/armor-up', function()
    SetPedArmour(PlayerPedId(), 100.0)
end)

RegisterNetEvent("3q_admin/client/play-sound", function(Sound)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, Sound, 0.3)
end)

RegisterNetEvent('3q_admin/client/DeletePlayerBlips', function()
   
    DeletePlayerBlips()
end)

RegisterNetEvent('3q_admin/client/UpdatePlayerBlips', function(Data)
