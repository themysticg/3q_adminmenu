...
RegisterNetEvent("3q_admin/client/send-report", function(ReportData)
...
RegisterNetEvent("3q_admin/client/close-report", function()
    if HasReport() then
...
RegisterNetEvent("3q_admin/client/reply-report", function(Message, Time)
    if HasReport() then
        local ReportId = GetPlayerReportFromName()
        if ReportId then
...
RegisterNetEvent('3q_admin/client/sync-chat-data', function(Type, Data, UpdateDelay)
