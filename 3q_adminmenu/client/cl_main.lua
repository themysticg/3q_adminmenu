...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('3q_adminmenu/server/get-permission', function(pGroup) 
        Group = pGroup
    end)
    Citizen.Wait(250)
    InitAdminMenu()
...
RegisterCommand('o_adminmenu', function(source, args, RawCommand) TriggerServerEvent('3q_admin/server/try-open-menu', true) end, false)

RegisterNetEvent('3q_admin/client/try-open-menu', function(KeyPress)
        if KeyPress then if not CanBind() then return end end
    ToggleMenu(false)
end)

RegisterNetEvent("3q_admin/client/reset-menu", function()
        ResetMenuKvp()
...
RegisterNetEvent("3q_admin/client/force-close", function()
    SendNUIMessage({ Action = 'Close', })
end)

RegisterNetEvent("3q_admin/client/toggle-debug", function()
...
RegisterNetEvent("3q_admin/client/do-perms-action", function(Action, CommandId, Group)
...
RegisterNUICallback("UnbanPlayer", function(Data, Cb)
    TriggerServerEvent('3q_admin/server/unban-player', Data.PData.BanId)
...
RegisterNUICallback('GetCharData', function(Data, Cb)
    QBCore.Functions.TriggerCallback('3q_admin/server/get-player-data', function(PlayerData)
        Cb(PlayerData)
    end, Data.Identifier)
end)
...
RegisterNUICallback("GetDateDifference", function(Data, Cb)
    QBCore.Functions.TriggerCallback('3q_admin/server/get-date-difference', function(FBans, CAmount)
        Cb({
            Bans = FBans, 
            Amount = CAmount,
        })
...
RegisterNUICallback("DeleteReport", function(Data, Cb)
    ...
    if Success then
        TriggerServerEvent('3q_admin/server/send-chat-report', ServerId, 
...
RegisterNUICallback('SendChatsMessage', function(Data, Cb)
    ...
    if ChatChannel == 'Staffchat' then
        ...
        TriggerServerEvent('3q_admin/server/sync-chat-data', ChatChannel, Config.StaffChat, 0)
    else
        ...
        if Report then -- Check if report exists
            if Config.Reports[Report] ~= nil then
                ...
                TriggerServerEvent('3q_admin/server/send-chat-report', Config.Reports[Report]['ServerId'], 
                '\n^0───────────────────────────\n^1[Admin] ^0→ ^5'..
                QBCore.Functions.GetPlayerData().name..'^0: '..ChatMessage..'\n^7→ '..
                Lang:t('info.report_reply', { chatcommand = Config.Commands['ReportChat'] })..
                '\n^0───────────────────────────')
                TriggerServerEvent('3q_admin/server/sync-chat-data', ChatChannel, Config.Reports, 0)
            end
        end
    end
    Cb("Ok")
