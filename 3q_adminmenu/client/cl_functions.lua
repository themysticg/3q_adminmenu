...
function SetKvp(Name, Data, SetData)
    ...
    SetResourceKvp('3q_adminmenu-'..Name, json.encode(Config.ResourceKVPs[KvpId]['Value']))
    Wait(100)
    RefreshMenu(Name)
end
...
function ResetMenuKvp()
    for i = 1, #Config.ResourceKVPs do
        SetResourceKvp("3q_adminmenu-" .. Config.ResourceKVPs[i]['Name'], "[]")
...
function RefreshMenu(Type)
    ...
    if Type == 'all' then -- Generate All
        for i=1, #Config.ResourceKVPs do
            ...
            if GetResourceKvpString("3q_adminmenu-"..KVP['Name']) == nil or GetResourceKvpString("3q_adminmenu-"..KVP['Name']) == "[]" then
                KVP['Value'] = GenerateKVP(KVP['Name'])
                SetResourceKvp("3q_adminmenu-"..KVP['Name'], json.encode(KVP['Value']))
            else
                KVP['Value'] = json.decode(GetResourceKvpString("3q_adminmenu-"..KVP['Name']))
            end
        end
...
function CreateLog(Type, Log, Data)
    ...
    QBCore.Functions.TriggerCallback('3q_admin/server/create-log', function(result)
    end, Type, Log, Data)
end
        ...
        for i = 1, #Config.Reports do
            ...
            if Report['Id'] == ReportId then
                SetTimeout(500, function()
                    ...
                    TriggerServerEvent('3q_admin/server/sync-chat-data', 'Reports', Config.Reports, 1500)
                end)
...
function GetServerConvar(Convar)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('3q_adminmenu/server/get-convar', function(ReturnData)
        Prom:resolve(ReturnData)
    end, Convar)
    return Citizen.Await(Prom)
end
...
function GetPlayersInArea(Coords, Radius)
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback('3q_admin/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("3q_admin/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("3q_admin/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("3q_admin/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end
